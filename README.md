...
`Chaos` 是一个以 10 人 AI 生存博弈为核心的叙事项目。产品目标不是传统命令菜单游戏，而是一个由世界时钟驱动、以聊天会话承载公开交流与私聊的多角色互动系统。

当前仓库已经改为 Web MVP：后端使用 Python 标准库提供 HTTP 服务，前端是单页聊天界面。它承接了第 1 轮《诱饵均值》的现有玩法，并按 PRD 先落地了公共大厅与私聊窗口（含主持人私聊）、轮询式状态同步和轻量后台调度。

## 当前实现

- 当前对局载入 10 名参赛者（001 玩家 + 9 名 NPC）
- 固定玩家开场设定
- 读取 `key.yaml` 并接入火山引擎 Responses API
- 使用 Responses API 缓存共享前缀提示词
- 环境转场固定文本（不接 LLM、不可私聊）
- 主持人宣读规则前的自由社交
- 主持人宣读规则后的第 1 轮执行
- 玩家通过 `speak` 与角色互动，可与主持人私聊确认规则
- 玩家秘密提交数字
...
- 前端采用聊天软件式 UI，区分公共大厅和私聊会话（含主持人私聊）
- 前端只展示玩家可见的信息，不暴露全部底层消息流
- 主持人与环境文本都进入统一消息系统；环境文本仅在转场出现且不作为独立 agent
