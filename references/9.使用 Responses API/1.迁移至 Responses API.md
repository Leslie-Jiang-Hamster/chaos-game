...
![图片](https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/aba4522e4aab46318574c8c3e460d20b~tplv-goo7wpa0wc-image.image)
![图片](https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/aba4522e4aab46318574c8c3e460d20b~tplv-goo7wpa0wc-image.image)
...
* 输入：Chat API 需要传入一个消息（messages）数组，而 Responses API 则接受字符串或数组格式的输入。同时，在 Responses API 中，支持使用 **instructions** 字段在特定轮次中补充系统提示词，使用方式请参见[补充系统提示词](2.文本生成.md#补充系统提示词)。
...
* Chat API 是无状态的，每次请求时需要将历史信息放在 **messages** 中，并通过 **role** 字段设置，以便进行主题相关的延续性对话。具体使用参见[多轮对话](../1.文本生成.md#多轮对话)。
* Responses API 默认开启存储功能，方便进行上下文管理，通过 **previous_response_id** 引入对应请求的输入和回复，实现智能交互体验。具体使用参见[上下文管理](2.文本生成.md#上下文管理)。
...
* Chat API：**response_format**，具体使用参见[结构化输出(beta)](../7.进阶使用/9.结构化输出(beta).md)。
* Responses API：**text.format**，具体使用参见[结构化输出(beta)](7.结构化输出(beta).md)。
...
> ✍️ 已移除 Chat API 列，省略 17 行。



### 更新最大输出长度参数

* Chat API：通过参数 **max_completion_tokens** 控制模型最大输出长度，具体教程参见[设置最大输出长度](../7.进阶使用/1.深度思考.md#31ecc4d7)。
* Responses API：通过参数 **max_output_tokens** 控制模型最大输出长度，具体教程参见[设置最大输出长度](3.深度思考.md#设置最大输出长度)。
...
### 使用上下文缓存能力
Context API 支持上下文缓存能力，而 Responses API 在缓存操控方面更加灵活，支持进行 ID 粒度的使用及变更。关于两种使用方式，参见[原理及选型](../7.进阶使用/5.上下文缓存.nd/1.原理及选型.md)。
...
#### 更新函数定义
Responses API 和 Chat API 在定义 function 函数方面有细微区别，具体使用教程参见[函数调用 Function Calling](../6.工具调用/7.函数调用 Function Calling.md)。
...
Chat API 当前不支持使用方舟大模型内置工具（联网搜索、图像处理、私域知识库搜索）、云部署 MCP等能力，可以通过 Responses API 使用，具体教程参见[联网搜索 Web Search ](../6.工具调用/3.联网搜索 Web Search.md)、[图像处理 Image Process](../6.工具调用/4.图像处理 Image Process.md)、[私域知识库搜索 Knowledge Search](../6.工具调用/5.私域知识库搜索 Knowledge Search.md)、[云部署 MCP / Remote MCP](../6.工具调用/6.云部署 MCP, Remote MCP.md)。
