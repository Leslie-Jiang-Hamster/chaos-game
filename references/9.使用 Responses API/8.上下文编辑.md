# [上下文编辑](https://www.volcengine.com/docs/82379/2123215?lang=zh)
上下文编辑功能可以管理上下文中思维链内容及工具调用内容，具备以下优势：
...
|功能策略 |标识符 |描述 |
|---|---|---|
|思维链内容清除 |`clear_thinking` |在开启思考时管理思维链内容。|
| | |该策略会自动清除先前轮次较旧的思维链内容。 |
...
# 组合策略
可以同时使用思维链内容清除和工具调用内容清除，使用多个策略时：

* 定义顺序：`clear_thinking` 必须排在 `clear_tool_uses` 之前。
* 处理顺序：先处理思维链内容清除，再处理工具调用内容清除。
