# [工具概述](https://www.volcengine.com/docs/82379/1827538?lang=zh)
...
* [图像处理 Image Process](4.图像处理 Image Process.md)
...
* [私域知识库搜索 Knowledge Search](5.私域知识库搜索 Knowledge Search.md)
...
### 函数调用 Function Calling
您也可以在创建模型响应时，使用`tools`来定义自定义函数。模型通过调用自定义函数代码，来访问模型中无法直接使用的特定数据或功能。了解更多信息，请参见[函数调用 Function Calling](7.函数调用 Function Calling.md)。

### 云部署 MCP / Remote MCP
Responses API支持通过 Streamable HTTP 链接的 MCP 调用。适用于复杂任务（如多步数据查询 + 分析）场景，支持与自定义 Function、Web Search 工具混合使用。详情请参见[云部署 MCP / Remote MCP](6.云部署 MCP, Remote MCP.md)。
