# [函数调用 Function Calling](https://www.volcengine.com/docs/82379/1262342?lang=zh)
...
### 工作原理图
![图片](https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/cf700fefc5204e8a8ab7e6bf3e5d3cd8~tplv-goo7wpa0wc-image.image) 
...
## 支持模型
全量支持函数调用的模型，请参见[函数调用能力](https://www.volcengine.com/docs/82379/1330310#6e3339cf)。

## 使用流程

### 支持 API
Function Calling 功能支持通过 [Chat API](https://www.volcengine.com/docs/82379/1494384?lang=zh) / [Responses API](https://www.volcengine.com/docs/82379/1569618?lang=zh) 进行调用，本文以 Chat API 调用为例。更多详情请参见[迁移至 Responses API](../9.使用 Responses API/1.迁移至 Responses API.md)。
...
更多关于函数 构造相关的规范和注意事项，请参见 [附1：工具函数参数构造规范](#附1工具函数参数构造规范)。
...
1. 推荐优选`doubao-seed-1.6`系列模型。
   1. FC场景下，关闭 thinking 会提高效率，具体操作请参见[开启/关闭深度思考](../7.进阶使用/1.深度思考.md#开启关闭深度思考)。
...
3. 常规调优手段：
   ...
   * 对模型进行sft（建议至少50条数据，模型越多、参数越多、情况越多，则所需要的数据越多），见[效果优化](#效果优化)。
...
启用严格模式需要满足以下前提条件（参见[结构化输出(beta)](../7.进阶使用/9.结构化输出(beta).md)）：

1. 在 `properties` 中定义的所有字段，都必须在 `required` 数组中进行声明。
2. 在函数的 `parameters` 定义中，推荐设置 `"additionalProperties": false`。
...
![图片](https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/71d9c354a2db4bd8a1959f43043cb944~tplv-goo7wpa0wc-image.image) 
...
* **进行模型精调**：通过监督微调（SFT）或强化学习，可以针对性地提升模型表现。详情请参见[模型精调概述](https://www.volcengine.com/docs/82379/1099459)。精调主要能改善以下方面：
...
本文主要讲解如何构造 Function Calling 工具，关于工具函数的使用请参考[基本使用流程](#基本使用流程)。
